...
- 如果用表来存储用户签到的消息，则会造成表数据的巨大增长，因此不适合存放在数据库中
- 使用bitMap位图法，把每一个bit位对应当月的每一天，形成映射关系，用0和1标示业务状态
- Redis中利用String类型数据机构实现bitMap，因此使用opsForValue来操作bitMap

###### 1、利用bitMap实现签到功能

- 当前登录用户和当前日期可以直接获取，因此不用传参
- 以月份为签到的一个基准，标明是某用户在某个月的签到记录
- 以天数为签到单位，获取签到日是该月的第几天
- 使用opsForValue进行签到，且签到成功为true
...
- 得到本月份中，到今天为止的所有签到数据
    ...
    ```shell
    BITFIELD key GET ufoo offset
    #u代表无符号位 foo表示查多少位 offset表示开始下标
    ```

- 得到数据后，遍历该数据并每个数据与1做与运算，运算后的数据即为签到数据本身

- 每完成一次遍历，就将数字右移一次，抛弃最后一个bit位，下一个bit位当成新的最后一个bit位



##### 十、UV统计模块（未实现）

- UV：Unique Visitor，独立访客量，指通过互联网访问、浏览这个网页的自然人。如1天内同一个用户多次访问该网站，只记录1次
- PV：Page View，页面访问量或点击量，用来衡量网站的流量。如用户每访问网站的一个页面，记录一次PV，用户多次打开页面，则记录多次PV
- HLL：HyperLoglog，概率算法，用于确定非常大的集合的基数，而不需要存储其所有的值（有约0.81%的误差），且Redis中的TTL基于String结构实现，Redis中的PFADD可以用来做UV统计，不记录重复数据

###### 1、实现UV统计的方案

- 每次有用户访问时，直接将用户信息放入HLL中，即使下次重复访问，HLL也会自动的过滤掉重复的元素
- 当以天为单位统计了一个月时，可以将一个月的数据合并成按月为单位的数据
...
- 设计key结构
    - 基本格式：[业务名称]：[数据名]：[id]
        - 避免key冲突，可读性强
    - 长度不超过44字节
        - 因为key是string类型，底层编码包含int、embstr和raw，且embstr在小于44字节时使用，采用连续内存空间，内存占用小，大于44时会变成raw编码，采用链表形式
    - 不包含特殊特字符
- BigKey问题
    - BigKey是指key的大小和key中的成员数量来判定，如：
        - key中的value值过大：一个String类型的key，其value大小是5mb
        - key中成员过多：一个ZSET类型的key，其成员数量有10000个
        - key中成员的数据量过大：一个Hash类型的key，成员数量只有100个，但是每个成员的大小为1M，则总和就有100M
    - 推荐单个key的value小于10kb
    - 对于集合类型的key，元素数量应小于1000
    - 危害：
        - 网络阻塞（占用大量带宽）、数据倾斜、Redis阻塞（Redis是单线程的）、CPU压力
    - 如何发现BigKey：
        - redis-cli--bigkeys、scan扫描、第三方工具、网络监控
    - 删除BigKey时，应使用unlink命令，异步的去删除
- 选择合适的数据结构
    - 如存储对象时，可以尝试采用Hash结构去存储
    - string类型几乎没有优化，且存储了过多的元信息，因此需要具体分析使用string
    - Hash结构的entry（键值对）数量不要超过500
    - 设置合理的超时时间
...
- 单机模式下的批处理（PipeLine）
    ...
    - N条命令依次执行时
        - N次命令的响应时间=N次往返网络传输耗时 + N次Redis执行命令耗时
    - N条命令批量执行时
        - N次命令的响应时间=1次往返网络传输耗时 + N次Redis执行命令耗时
        - 但是不要在一次批处理中传输 太多命令，否则单次命令占用带宽过多，会导致网络阻塞
    - 使用MSET可以批处理，但是只能操作部分的数据类型，因此如果对复杂数据类型进行批处理，则可以使用pipeline，将批处理命令批量放入到管道，然后批量执行
    - pipeline的多个命令不具备原子性
