...
- 正常情况下，释放锁只需要手动释放即可
- 在服务宕机情况下，需要给锁设置一个超时时间，以防出现业务阻塞的情况
...
- 非阻塞式：尝试一次获取锁，获取成功返回true，否则返回false，而不会进行忙等

    ```shell
    #添加锁 NX是互斥 EX是设置超时时间
    SET lockFoo threadBar NX EX 10
    ```
...
- 进程一获得锁之后业务没有结束，触发了超时释放，此时进程二可以获取到锁
- 进程一业务结束之后，会主动释放锁，即执行DEL操作，但此时删除的锁是进程二所持有的，因此发生了误删操作
- 可以在主动释放锁时，进行判断是否该锁是属于自己的

###### 4、判断锁之后仍误删的情况

- 进程一正常执行完业务准备释放锁，在判断完该锁已经属于自己的之后，还没来得及释放锁就被其他线程阻塞，从而导致了锁的超时释放
- 此时进程二进入并成功获取锁，如果此时进程一执行，已经不用判断锁的归属，会直接将线程二的锁误删
- 因此需要保证判断和释放锁操作的原子性（可以采用Lua脚本的方式，Redis会原子性的执行脚本中的指令）
